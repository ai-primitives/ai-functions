...
describe('RequestHandler', () => {
  ...
  it('should handle timeouts', async () => {
    ...
    const operation = vi.fn(
      () =>
        new Promise((resolve) => {
          globalThis.setTimeout(resolve, 2000, 'delayed result')
        }),
    )
  ...
  it('should not retry on non-retryable errors', async () => {
    const handler = createRequestHandler()
    const operation = vi.fn().mockRejectedValue(new AIRequestError('Non-retryable error', undefined, false))
